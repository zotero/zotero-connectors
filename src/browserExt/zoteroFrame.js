 ...
 * Used by browser extension pages that need to communicate with an isolated frame.
 */
Zotero.Frame = class ZoteroFrame {
	...
	async _initMessaging(messagingOptions) {
		if (!messagingOptions.sendMessage) {
			...
			// The webpage can technically capture this and snoop messages, but we're not sending
			// sensitive data anyway. Callers can provide expectedHandshake to authenticate the
			// other end before normal messaging is established.
			this._frame.contentWindow.postMessage('zoteroChannel', '*', [mc.port2]);
			let handshake = await new Promise(cb => { mc.port1.onmessage = cb; });
			mc.port1.onmessage = null;
			if (messagingOptions.expectedHandshake !== undefined) {
				let expectedHandshake = messagingOptions.expectedHandshake;
				let isValid = typeof expectedHandshake === 'function'
					? await expectedHandshake(handshake.data)
					: handshake.data === expectedHandshake;
				if (!isValid) {
					throw new Error('Zotero.Frame: Invalid messaging handshake');
				}
			}
			// Established a 2-way messaging channel at this point
