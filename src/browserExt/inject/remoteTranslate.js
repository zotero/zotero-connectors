...
Zotero.RemoteTranslate = new function() {
	...
	// because translateHost strips it before parsing.
	const remoteHeadElements = new Set([
		'base',
		'link',
		'meta',
		'title',
	...
	function isInvalidHeadChild(node) {
		return (node.nodeType == Node.ELEMENT_NODE && !remoteHeadElements.has(node.localName))
	...
	function serializeDocument(document, includeCookie) {
		...
		if (head && [...head.childNodes].some(isInvalidHeadChild)) {
			...
			let rootHTML = document.documentElement.cloneNode(false).outerHTML;
			let closingTag = `</${document.documentElement.localName}>`;
